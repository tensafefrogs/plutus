...
runEmulatorTraceIO'
    ...
    -> EmulatorTrace ()
    -> IO ()
runEmulatorTraceIO' tcfg cfg trace = do
  let (xs, me, e) = runEmulatorTrace cfg trace
      printLn     = hPutStrLn (outputHandle tcfg)
   in do
      case me of
        Nothing  -> return ()
      ...
      printBalances printLn (balances e)

pad :: Int -> Integer -> String
pad n k = (\x -> replicate (n - length x) '0' ++ x) (show k)
...
flattenValue v = do
    (cs, m) <- AMap.toList $ getValue v
    (tn, a) <- AMap.toList m
    guard $ a /= 0
    return (cs, tn, a)

printBalances :: (String -> IO ())
              -> Map.Map Entity Value
              -> IO ()
printBalances printLn m = do
